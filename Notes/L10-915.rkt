...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname L10-915) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
(require 2htdp/universe)
;; What is the world state:

(define-struct crash [x v])
;; A Crash is a (make-crash x v)

(define-struct twoboats [x1 vx1 x2 vx2])
;; A TwoBoats is a (make-twoboats Number Number Number Number)
;; - x1 x position of first boat in pixels from the left
;; - vx1 velocity of the first boat in pixels/tick (moving to the right)
;; - x2 x position of second boat in pixels from the left
;; - vx2 velocity of the second boat in pixels/tick (moving to the left)
...
;; simulate a boat collision
(define (collision init-tb)
  (big-bang init-tb
    [to-draw draw-twoboats]
    [on-tick move-twoboats]))


(define BOAT-1 (rectangle 20 10 "solid" "blue"))
(define BOAT-2 (rectangle 20 10 "solid" "red"))
(define Y-BOAT 150)
(define BACKGROUND (empty-scene 800 300))
;; draw-twoboats : TwoBoats -> Image
;; renders the two boats at the given positions
...
;; - #false
;; - Natural Number
;; Interpretation: a student's NUID
;; or #false if it is not known
...
;; moon-run : MP -> MP
;; animates a moon moving across the sky, with key input
(define (moon-run mp)
  (big-bang mp
    [to-draw draw-moon]
    [on-key key-moon]
...
;; starts a "waiting" MP moving on the left side of the screen
