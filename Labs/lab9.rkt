...
;; Signature: large-withdrawls : ATMHistory -> Number
;; Purpose Statement: the function looks to see if the account is withdrawable and then it withdraws
;; all the numbers that are greater than a 1000.
;; Tests:
(check-expect (large-withdrawls ATM-HISTORY-EX-1) '())
(check-expect (large-withdrawls ATM-HISTORY-EX-2) '())
(check-expect (large-withdrawls ATM-HISTORY-EX-3) (list TRANSACTION-EX-3 TRANSACTION-EX-4))
;; Code:
(define (large-withdrawls n)
  (filter (lambda (d)
            (and (>= (transaction-amount d) 1000)
                 (transaction-is-withdraw d)))
          n))
...
;; Estimated Portion of Lab: 20%
;; Signature: sum-transactions : ATMHistory -> Number
;; Purpose Statement: the function takes in a ATM's transaction history and adds all the transactions up
;; Tests:
(check-expect (sum-transactions ATM-HISTORY-EX-1) 0)
(check-expect (sum-transactions ATM-HISTORY-EX-2) 175)
;; Code:
(define (sum-transactions history)
  (foldr (lambda (transaction n)
           (+ (transaction-amount transaction) n))
         0
         history))
...
;; Signature: transactions-within-time : ATMHistory StartDateTime EndDateTime -> LoT
;; Purpose Statement: the function thats an ATM's transaction history and returns the transcations between the
;; start and end time, inclusive
;; Tests:

;; Code:
(define (transactions-within-time history start end)
  (filter (lambda (transaction)
           (and (>= (seconds-since-1990 (transaction-datetime transaction))
                    (seconds-since-1990 start))
                (<= (seconds-since-1990 (transaction-datetime transaction))
...
;; LePage's argument is inaccurate beacause even though they withdrew cash from an ATM within a smoke shop
;; liquor store, or an out of state location, that doesn't neccesarily mean that they spent that money on items
;; that are tehcnically not eligible under EBT. Even though it could be considered a reasonable assumption it is not fair
;; to set that in stone. 
...
;; Estimated Portion of Lab: 10%
;; One technical propsal would be to implment an algorithm that checks the past spending habits of the user
;; and use that in comparison to what they just purchased to see if it is typical for them. A non technical proposal would be
;; to maybe allow the user to choose the way they spend on an app or something simillar. 
