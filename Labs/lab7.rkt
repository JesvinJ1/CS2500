...
;; longitudinal position, diameter (in feet), and whether repairs are urgently needed.
...
(define (pothole-temp pd)
  (... (pothole-x pd) ... (pothole-y pd) ... (pothole-diameter pd) ... (pothole-urgent? pd) ...))
...
;; Signature: urgent-potholes : DriveData -> DriveData
;; Purpose Statement: the function takes in a DriveData and returns a DriveData returning a list of all true values for urgent
;; Tests:
...
;; Code:
(define (urgent-potholes u)
  (cond
    [(empty? u) '()]
    [(cons? u)
     (cond [(pothole-urgent? (first u))
            (cons (first u) (urgent-potholes (rest u)))]
           [else
            (urgent-potholes (rest u))])]))
...
;; Estimated Portion of Lab: 20%
;; Another field could be added that adds a range for example of 1-10 that specifies how urgent the repair is.
;; The number would represent in a sense the size and how bad the pothole is. 
...
;; This system was only possible if you could download an app on your smartphone. As mentioned, if you
;; couldn't afford a smartphone then you would not be able to download the app and report potholes.
;; This meant that this initiative excluded people that could not afford to get a phone and as a result
;; could not participate in this initiative. 
...
;; Estimated Portion of Lab: 20%
;; Signature: total-pothole-size : DriveData -> Number
;; Purpose Statement: it adds the two diameters in feet and returns the sum which is the size of the pothole and in this case
;; the amount of cement that is needed
;; Tests:
...
;; Code:
(define (total-pothole-size p)
   (cond
    [(empty? p) 0]
    [(cons? p) (+ (pothole-size (first p)) (total-pothole-size (rest p)))]))
...
;; Signature: average-pothole : DriveData -> MaybeNumber
;; Purpose Statement: the function takes in a DriveData and retuerns the average dimater of the pothole
;; and if there are no potholes it just returns false
;; Tests:
(check-expect (average-pothole DD-EX-1) #f)
...
(define (average-pothole a)
  (cond
    [(empty? a) #f]
    [(cons? a) (/ (total-pothole-size a) (size-list a))]))